                            ...
                            <form class=""  method="post" autocomplete="off" name="CreateCompanyProductForm" id="CreateCompanyProductForm" action="action/CreateCompanyProduct.php" novalidate>
                                ...
                                <div class="field item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <input class="form-control"  name="SerialNo" id="SerialNo" placeholder="" required="required" autofocus /><span id="MSG_SerialNo"></span>
                                    </div>
                                </div>                           
                                <div class="field item form-group">
                                 ...
                                 <div class="field item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                      <select id="ProductsDetails" name="ProductsDetails[]" class="form-control clsProducts">
                                ...
                                <div class="field item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6">
                                        <input class="form-control"  name="Email" id="Email" placeholder="" required="required"  />
                                    </div>
                                </div> 
                                <div class="field item form-group">
                                ...
                                <!-- <div class="ln_solid"> -->
                                    <div class="form-group">
                                        <div class="col-md-6 offset-md-3">
                                            ...
                                            <button type='button' class="btn btn-success"><a  style="color:white;" href="Companies_Products.php?action=View">View</a></button>  
                                        </div>
                                    </div>
                                <!-- </div> -->
                            </form>
            ...
            <div class="row">
              ...
              <!-- <form class="form-inline">
                ...
                <br><br><br>
                    ...
                    <table id="SrchTable" class="table table-striped jambo_table bulk_action css-serial" >
                      <thead>
                        <tr>    
                          ...
                          <th style="width:20%;Display:none;">Email Id</th>
                          <th style="width:7%;text-align:left;">Mob No.</th>
                          <!-- <th>Amc's From</th>
                          <th>Amc's To</th> -->
                          <th style="width:7%;">AMC's From</th>
                           <th style="width:7%;">AMC's To</th>
                          <th style="width:5%;">Service</th>
                          <th style="width:17%;Display:none;">Description</th>
                        ...
                        <div class="x_content">
                            <form class=""  method="post" autocomplete="off" name="UpdateCompanyProductForm" id="UpdateCompanyProductForm" action="action/CreateCompanyProduct.php" novalidate>
                                ...
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Companies<span class="required">*</span></label>
                                    <div class="col-md-6 col-sm-6">
                                      <select id="frameworrk" name="framework[]"  class="form-control clsframework">
                                ...
                                <div class="field item form-group">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align">Not Interested</label>
                                    <div class="col-md-6 col-sm-6">
										<input type="checkbox" name="NotInterested" id="NotInterested" value="0" style="margin-top:10px;">
                                    </div>
                                </div>
...
$.ajax({
  ...
  success: function (data) {
    ...
    $.each(ProductData, function(i,val) {
        if(localStorage.Product == val.Product_Name){
...
$.ajax({
    ...
    // $.get("action/CreateCompany.php",{action:'Display_Company_Data'},function(data, status){
      ...
      let Status = "";
      let DeleteView = 0;
      let Statuss     = '<?php echo $_SESSION["user_type"] ?>';
      if(Statuss == "admin"){
          DeleteView = 1;
      }
              //alert(JSYear);
            ...
            $.each(ActualData, function(i,val) {
              ...
              if(Curr_Date > val.Amc_Expire_On && val.ServiceStatus==0){
                Status = '<span style="color:red;font-weight:bold;">Not Intrsted</span>';
              }
              
              if(val.Amc_Expire_On == null && val.Amc_From_Date == null){
                Status = '<span style="color:red;font-weight:bold;">No Amcs</span>';
              }
              
              if(val.Amc_Expire_On == null && val.Amc_From_Date == null && val.ServiceStatus==0){
                Status = '<span style="color:red;font-weight:bold;">Not Intrsted</span>';
              }
              
                  htmlData+=`<tr id="row_` + val.Prodct_Compny_Id + `">
                  <td style="width:4%;text-align:center;">&nbsp;</td>
                  <td style="width:20%;" id="IDCompanyName"><a href="#"><b>`+val.CompanyName+`</b></a></td>
                  <td style="width:12%;" id="IDProduct_Name">`+val.Product_Name+`</td>
                  <td style="width:10%;" id="IDSerialNo"><a href="CreateProduct_Amcs.php?action=View" class="txtlink" onclick="ProductID(`+val.Prodct_Compny_Id+`)">`+val.Serial_No+`</a></td>
                  <td style="width:0%;Display:none;" id="IDEmail_id" >`+val.Email_id+`</td>
                  <td style="width:7%;">`+val.ContactNo+`</td>                
                  <td style="width:7%;">`+AMCFromDate+ `</td>
                  <td style="width:7%;">`+AMC_Expire_Date +`</td>
                  ...
                  <td style="width:0%;Display:none;" id="IDDescription">`+val.Description+`</td>
                  <td style="Display:none;" id="IDNotInterested">`+val.ServiceStatus+`</td>
                   <td id="IDAction" style="width:5%;padding-left: 40px !important;"><center>
                    <div class="btn-group" style="display: block !important;">
                      <i class="fa fa-mail-forward" dropdown-toggle" data-toggle="dropdown"></i>
                      <div class="dropdown-menu">`;
                        if(DeleteView == 1){
                             htmlData+=`<a class="dropdown-item delete" href="#" data-formindex="`+ (val.Prodct_Compny_Id) + `">&nbsp; Delete Product</a>`;
                        }
                        htmlData+=`<a class="dropdown-item edit" data-toggle="modal" data-target="#myModal" href="#" data-formindex="`+ (val.Prodct_Compny_Id) + `">&nbsp; Edit Product</a>
                         <a class="dropdown-item clsAmcs" data-toggle="modal" data-target="#AMCModal" href="#" data-formindex="`+ (val.Prodct_Compny_Id) + `">&nbsp; Create Subscription</a>
                      </div></center>
                   </td>
...
// <td style="width:10%;" id="IDAction"><button class="btn btn-success delete"  style="height: 22px !important;
...
//                   </td>
 ...
 function ProductID(PID,Company,Product,Description,Email){
...
//   localStorage.setItem("Product",  Product);
     $('#SerialNo').val(localStorage.SerialNo);
     $('#Email').val(localStorage.Emailid);
     $('#Description').val(localStorage.Remark);
...
$('#UpdateCompanyProductForm').submit(function (e) {
    ...
    if($('#SerialNo').val() != "" && $('.clsframework').val() != "" && $('#ProductsDetails').val() != "" ){
          ...
          var formData = {        
              ...
              'Email'            : $('#Email').val(),
              'Description'      : $('#Description').val(),
              'NotInterested'    : $('#NotInterested').val(),
...
$(document).on('click','.edit',function () {
  //GET DATA
      ...
      let NotInterested    = $(this).closest('tr').find('#IDNotInterested').html();
    ...
    //SET DATA
      $.ajax({
        type: 'POST',
        url: 'action/SelectCompany.php',
        data: {action:'Display_CompanyName_Data_For_CompanyProduct'},
        success: function (data) {
          let CompanyData = JSON.parse(data);
             $.each(CompanyData, function(i,val){
                 if(CompanyInsertId == val.CompanyInsertId){
                    htmll+=`<option value="`+CompanyInsertId+`" Selected>`+CompanyName+`</option>`;
                }else{
    ...
    //     success: function (data) {
    //       let CompanyData = JSON.parse(data);
    //      // htmll+=`<option value="`+Product_Insert_Id +`">`+Product_Name+`<option>`;
    //       $.each(CompanyData, function(i,val) {
    //           htmll+=`<option value="`+val.CompanyInsertId+`">`+val.CompanyName +`</option>`;
    //       });
    //       $(document).find(".clsframework").html(htmll);
...
// NotInterested
//  $('#NotInterested').onchange(function() {
